const e=`<template>
  <div class="table-content-container !border-none">
    <div class="table-content-header !px-0">
      <table-sort-columns
        :save-key="productsGroupHeader"
        :templates="productGroupsActiveStore.templates"
        @onChangeTableHeaders="onChangeTableHeaders"
      />
      <ShowHideColumn
        :headers="productGroupsActiveStore.templates"
        :save-key="productsGroupHeader"
      />
      <page-size-btn
        :current-size="productGroupsActiveStore.params.page_size"
        :total-count="productGroupsActiveStore.data?.total_count"
        :page-number="productGroupsActiveStore.data?.page_number"
        @setPageSize="productGroupsActiveStore.setPageSize"
      />
      <search-input @change="productGroupsActiveStore.search" />
      <excel-btn
        @click="productGroupsActiveStore.onDownloadExcelFile"
        :loading="productGroupsActiveStore.isExcelFileDownloading"
      />
      <RefreshBtn
        @click="refresh"
        :loading="productGroupsActiveStore.loading"
      />
    </div>
    <div class="table-content-body">
      <data-table
        :headers="productGroupsActiveStore.templates"
        :sorted="productGroupsActiveStore.params.order_by"
        :loading="productGroupsActiveStore.loading"
        :is-empty="!productGroupsActiveStore.data?.items.length"
        @sort="productGroupsActiveStore.sortData"
      >
        <template #body>
          <c-tr
            v-for="data in productGroupsActiveStore.data?.items"
            :key="data"
          >
            <c-td-no-edit
              v-for="key in productGroupsActiveStore.templates"
              :key="key"
              :is-checked="key.checked"
              :type="key.type"
            >
              <div v-if="key.key === 'action'">
                <rounded-icon-btn
                  type="edit"
                  :iconSize="20"
                  @click="openEditDialog(data?.id)"
                />
              </div>
              <div v-else>
                {{ data[key.key] }}
              </div>
            </c-td-no-edit>
          </c-tr>
        </template>
      </data-table>
    </div>
    <div class="table-content-footer">
      <curren-page-btn
        :current-size="productGroupsActiveStore.params.page_size"
        :total-count="productGroupsActiveStore.data?.total_count"
        :page-number="productGroupsActiveStore.data?.page_number"
      />
      <page-index
        :available-pages="productGroupsActiveStore.data?.total_pages"
        :current-page="productGroupsActiveStore.data?.page_number"
        @setPage="productGroupsActiveStore.setPage"
      />
    </div>
  </div>
  <transition name="modal">
    <div v-if="editingId">
      <SettingsProductsGroupsNewGroup
        :id="editingId"
        @close-dialog="closeEditDialog"
        @clear-fetched-tab="clearFetchedTab"
      />
    </div>
  </transition>
</template>

<script setup lang="ts">
import type { Template } from "~/interfaces/ui/template";
import { useEventBus } from "~/composables/EventBus/eventBus";
import { SettingsEventKeys } from "~/variable/event-key-constants";
import { productsGroupHeader } from "~/variable/column-constants";

// props
const props = defineProps<{
  isActive: boolean;
}>();

// emits
const emit = defineEmits(["clearFetchedTab"]);

// store
const { isActive } = toRefs(props);
const productGroupsActiveStore = useProductGroupsStore(
  isActive.value.toString(),
);

// states
const eventBus = useEventBus();
const editingId = ref<string>("");
const updateListEventKey = SettingsEventKeys.PRODUCT_GROUP_TABLE_UPDATE;

// hooks
eventBus.on(updateListEventKey, async (updatedIsActive) => {
  if (updatedIsActive === isActive.value) {
    await getData();
  }
});

onMounted(async () => {
  await getData();
});

// methods

const onChangeTableHeaders = (param: Template[]) => {
  productGroupsActiveStore.templates = param;
};

const openEditDialog = (id: string | undefined) => {
  if (!id) return;
  editingId.value = id;
};

const closeEditDialog = () => {
  editingId.value = "";
};

const clearFetchedTab = (isActive: boolean) => {
  emit("clearFetchedTab", isActive);
};

const refresh = async () => {
  await productGroupsActiveStore.refresh();
};

const getData = async () => {
  await productGroupsActiveStore.getData(isActive.value.toString());
};
<\/script>
`;export{e as default};
