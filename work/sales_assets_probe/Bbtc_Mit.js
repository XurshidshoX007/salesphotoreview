  ...
  <form id="app" @submit.prevent="save">
    ...
    >
      <flex-col class="gap-5">
        ...
        />
        <d-input
          :label="t('settings.bonus_percentage')"
          required
          :value="data.bonus"
          type="number"
          @change="data.bonus = $event"
        />
...
import type { RlpBonusModel } from "~/interfaces/api/settings/rlp-bonus-model";
...
const save = async () => {
  ...
  const res = await rlpBonusStore.add(data.value);
  if (res !== "error") {
...
const getDetail = async () => {
  initialDetailData.value = await rlpBonusStore.getDetail(props.id!);
