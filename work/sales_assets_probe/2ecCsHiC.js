const n=`<template>
  <form id="app" @submit.prevent="save">
    <d-modal
      ...
      :loading="expenseStore.isDetailExpenseCategoryLoadin"
...
import type { ExpenseCategoryListModel } from "~/interfaces/api/settings/expense/expense-model";
...
const data = ref<ExpenseCategoryListModel>({
  id: undefined,
  code: null,
  name: null,
  default_name: "",
...
const save = async () => {
  ...
  const res = await expenseStore.addExpenseCategory(data.value);
  if (res !== "error") {
...
const getDetail = async () => {
  initialDetailData.value = await expenseStore.getDetailExpenseCategory(
    props.id!,
  );
