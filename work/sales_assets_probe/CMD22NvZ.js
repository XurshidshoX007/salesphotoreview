...
import type { AccessBranchUnattachedUserModel } from "~/interfaces/api/access/branch-model";
...
// State
const dataList = ref<AccessBranchUnattachedUserModel[]>([]);
...
onMounted(async () => {
  try {
    ...
    const { data } = await branchStore.getUnattachedUsers();
    dataList.value = data;
  } catch (e) {
    console.log(e);
  } finally {
...
const onSave = async () => {
  ...
  if (checkedItemIds.value.length) {
    const res = await branchStore.attachUsers({
