    ...
    <div class="table-content-container">
      <div class="table-content-header !p-3 justify-between">
        <div class="table-content-btn-group">
          ...
          <page-size-btn
            ...
            :total-count="dashboardStore.clientCategoryData?.items?.length"
          ...
          <refresh-btn
            @click="refresh"
            :loading="dashboardStore.clientCategoryLoading"
          />
        </div>
      </div>
      <div class="table-content-body !p-0">
        <data-table
          :headers="headers"
          :loading="dashboardStore.clientCategoryLoading"
          :is-empty="!dashboardStore.clientCategoryData?.items?.length"
          :sorted="sorted"
          @sort="onSort"
        >
          <template #body>
            ...
            >
              <c-td-no-edit
                ...
                :type="column.type"
              >
                <template v-if="column.key === 'tcb_percent'">
                  {{ getFormattedPercent(item.tcb_percent) }}
                </template>
                <template v-else>
                  {{
                    getDataValue(
...
import { dashboardSalesClientCategoryTable } from "~/variable/column-constants";
...
const saveKey = dashboardSalesClientCategoryTable;
const headers = computed(() => dashboardStore.clientCategoryTemplates);

// Hooks
const tableData = computed(() => {
  return dashboardStore.clientCategoryData?.items || [];
});
...
const totals = computed(() => {
  const backendTotal = dashboardStore.clientCategoryData?.total;
  ...
  if (useBackendTotal) {
    ...
    return headers.value.reduce<Record<string, number>>((acc, column) => {
      if (column.type === "number" && column.key in backendTotal) {
        ...
        acc[column.key] =
          column.key === "tcb_percent" ? Math.round(val ?? 0) : (val ?? 0);
      }
      return acc;
    }, {});
  }
  ...
  return headers.value.reduce<Record<string, number>>((acc, column) => {
    if (column.type === "number") {
      ...
      acc[column.key] =
        column.key === "tcb_percent" ? Math.round(total) : total;
    }
    return acc;
  }, {});
});
...
const refresh = async () => {
  await dashboardStore.getClientCategory();
};
...
const downloadExcelFile = () => {
  if (!dashboardStore.clientCategoryData?.items?.length) return;

  const header = headers.value.reduce<Record<string, string>>((acc, item) => {
    acc[item.key] = item.name;
    return acc;
  }, {});
  const excelData = transforExcelData(
    dashboardStore.clientCategoryData.items,
    headers.value,
  );
...
const onChangeTableHeaders = (newHeaders: Template[]) => {
  dashboardStore.clientCategoryTemplates = newHeaders;
};
