...
import type { AccessPaymentMethodUnallowedUserModel } from "#imports";
...
// State
const dataList = ref<AccessPaymentMethodUnallowedUserModel[]>([]);
...
onMounted(async () => {
  try {
    ...
    const { data } = await paymentMethodStore.getUnallowedUsers();
    dataList.value = data;
  } catch (e) {
    console.log(e);
  } finally {
...
const onSave = async () => {
  try {
    ...
    if (checkedItemIds.value.length) {
      const res = await paymentMethodStore.allowPaymentMethods({
