...
import type { AccessWarehouseUnattachedUserModel } from "~/interfaces/api/access/warehouse-model";
...
// State
const dataList = ref<AccessWarehouseUnattachedUserModel[]>([]);
...
onMounted(async () => {
  try {
    ...
    const { data } = await warehouseStore.getUnattachedUsers();
    dataList.value = data;
  } catch (e) {
    console.log(e);
  } finally {
...
const onSave = async () => {
  ...
  if (checkedItemIds.value.length) {
    const res = await warehouseStore.attachUsers({
