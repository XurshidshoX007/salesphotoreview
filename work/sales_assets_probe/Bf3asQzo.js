const e=`<template>
  <div class="access-cashboxes-dialog-modal">
    <d-modal
      with-out-header
    ...
    >
      <template #header>
        ...
        >
          <span> {{ t("access.attach_cashbox") }}: </span>
...
import type { AccessCashboxCheckModel } from "~/interfaces/api/access/cashbox-model";
...
// State
const dataList = ref<AccessCashboxCheckModel[]>([]);
...
const nonDisabledFilteredIds = computed(() => {
  return (
    filteredData.value
      .filter(
        (item) => item.is_active || checkedItemIds.value.includes(item.id),
      )
      .map((item) => item.id) || []
  );
});
...
const isAllSelected = computed((): boolean => {
  return (
    nonDisabledFilteredIds.value.length > 0 &&
    nonDisabledFilteredIds.value.every((id) =>
      checkedItemIds.value.includes(id),
    )
  );
});

const isAllIndeterminate = computed((): boolean => {
  const someChecked = nonDisabledFilteredIds.value.some((id) =>
    checkedItemIds.value.includes(id),
  );
...
const onAllSelect = (isChecked: boolean) => {
  const currentSelected = checkedItemIds.value;
  const nonDisabledIds = nonDisabledFilteredIds.value;

  if (isChecked) {
    checkedItemIds.value = Array.from(
      new Set([...(currentSelected || []), ...nonDisabledIds]),
    );
  } else {
    checkedItemIds.value = (currentSelected || []).filter(
...
const onSaveAttach = async () => {
  ...
  try {
    await accessStore.onAttachCashboxes({
      user_id: accessStore.activeUserId,
      cashbox_id_arr: checkedItemIds.value,
    });
...
const getDataList = async () => {
  ...
  const cashboxes = await accessStore.getUserCashboxes();
  dataList.value = cashboxes || [];
...
<style lang="scss">
.access-cashboxes-dialog-modal .modal-body-content {
  margin: 0 -8px;
