...
const updateMarkers = async (
  ...
  if (!isMapClickEventAdded.value) {
    mapInstance.events.add("click", (e: any) => {
      ...
      if (target === mapInstance && !isHasGpsUpdated.value) {
        gpsUpdateFunction("current_route");
  ...
  const validClients = clients.filter(
    (client) => client?.lat_lng && isValidCoordinate(client.lat_lng),
  );
  ...
  validClients.forEach((client, index) => {
    ...
    const visitMarkerIndex = clients.findIndex((item) => item.id === client.id);
...
const gpsUpdateFunction = async (mapActionId: string) => {
  if (mapActionId === "current_route") {
    await updateMarkers(gpsStore.visitSequenceData);
    await getAllRouteByOSRM();
  } else {
    await updateMarkers(
      gpsStore.visitSequenceOptimizationData[props.weekDayItem?.type],
    );
  }
};
...
const getAllRouteByOSRM = async () => {
  const validPoints =
    gpsStore.visitSequenceData?.filter((item) => item?.lat_lng) || [];

  if (validPoints.length <= 1) return;

  const routeCoords = validPoints
    .map(({ lat_lng }) => \`\${lat_lng.longitude},\${lat_lng.latitude}\`)
    .join(";");
