const e=`<template>
  <div class="access-warehouses-dialog-modal">
    <d-modal
      with-out-header
    ...
    >
      <template #header>
        ...
        >
          <span> {{ t("access.attach_warehouse") }}: </span>
...
import type { AccessWarehouseCheckModel } from "~/interfaces/api/access/warehouse-model";
...
// State
const dataList = ref<AccessWarehouseCheckModel[]>([]);
...
const onSaveAttach = async () => {
  ...
  try {
    await accessStore.onAttachWarehouses({
      user_id: accessStore.activeUserId,
      warehouse_id_arr: checkedItemIds.value,
    });
...
const getDataList = async () => {
  ...
  const warehouses = await accessStore.getUserWarehouses();
  dataList.value = warehouses || [];
...
<style lang="scss">
.access-warehouses-dialog-modal .modal-body-content {
  margin: 0 -8px;
