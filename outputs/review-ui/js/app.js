const LS_MARKS='lmjDateReviewMarksV2',LS_REASONS='lmjCustomReasonsV2',LS_REASON_OVERRIDES='lmjReasonOverridesV1',LS_DELETED_REASONS='lmjDeletedReasonsV1',LS_DATE='lmjSelectedDateV2',LS_BRAND='lmjSelectedBrandV1',LS_THEME='lmjUiThemeV1',LS_METRICS='lmjPhotoMetricCacheV1',LS_PHOTO_PAGE_SIZE='lmjPhotoPageSizeV1',LS_CLIENT_ID='lmjReviewClientIdV1';
  ...
  return String(value??'')
    ...
    function showReasonContextMenu(x,y){
      ...
      menu.querySelectorAll('.reasonContextItem').forEach(button=>button.onclick=()=>{
        const reason=button.dataset.reason||'';
        const next=new Set(selectedReasonValues());
 ...
 ${rows.map(r=>`<tr height=20 style='height:15.0pt'>
  ...
  return String(v??'')
    ...
    window.addEventListener('load',()=>{
      ...
      if(modalImageBox){
        modalImageBox.addEventListener('touchstart',event=>{
          const touch=event.touches?.[0];if(!touch)return;
          reasonLongPressStart={x:touch.clientX,y:touch.clientY};
          reasonLongPress=setTimeout(()=>{showReasonContextMenu(reasonLongPressStart.x,reasonLongPressStart.y);reasonLongPress=null},550);
        },{passive:true});
        modalImageBox.addEventListener('touchmove',event=>{
          const touch=event.touches?.[0];if(!touch||!reasonLongPressStart||!reasonLongPress)return;
          if(Math.hypot(touch.clientX-reasonLongPressStart.x,touch.clientY-reasonLongPressStart.y)>12){clearTimeout(reasonLongPress);reasonLongPress=null}
        },{passive:true});
        ['touchend','touchcancel'].forEach(name=>modalImageBox.addEventListener(name,()=>{if(reasonLongPress)clearTimeout(reasonLongPress);reasonLongPress=null;reasonLongPressStart=null},{passive:true}));
      }
